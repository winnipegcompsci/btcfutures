...
var TradesModel = require('../app/models/trade.server.model.js');
var UsersModel = require('../app/models/user.server.model.js');
var ExchangesModel = require('../app/models/exchange.server.model.js')
...
function papertrade(amount, price, bias, type) {
    ...
    if(bias == "LONG") {
        ...
        lever_rate = 10;
        Exchange.findOne({"name": "OKCoin"}, function(err, xchg) {
           if(err) {
               console.log("Err: " + err);
           }
           console.log("FOUND EXCHANGE: " + xchg);
           thisExchange = xchg._id; 
            ...
            trade.save(function(err) {
                if(err) {
                    console.log("Debug:: Couldn't save trade to db");
                    console.log("Error: " + err);
                } else {
                    console.log("Debug:: Saved Trade to MongoDB");
                }
            });
    ...
    if(bias == "SHORT") {
        ...
        lever_rate = 20;
        Exchange.findOne({"name": "796"}, function(err, xchg) {
            if(err) {
                console.log("Err: " + err);
            }
            console.log("FOUND EXCHANGE: " + xchg);
...
if(fs.existsSync('papertrade_trades.txt')) {
    if(PAPERTRADE) {
        fs.readFile('papertrade_trades.txt', function read(err, data) {
            ...
            INITIAL_PROFITLOSS = CURRENT_PROFITLOSS;
            CURRENT_PROFITLOSS = (INITIAL_PROFITLOSS * -1)
